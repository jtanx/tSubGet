...
static void printHelp(wchar_t *exec){
	int i;
	wprintf(L"Usage:\n%s [-d delay] [-p page] [-o out] [-c] [-s col,cs] [-y|-k] [-l langid] fileIn\n", exec);
	...
	wprintf(L"\t[-k]\t\tSkips if the output already exists.\n");
...
static int parseArgs(ParserOpts *po, int *pos, int argc, wchar_t *argv[]){
	if (argv[*pos][0] == L'-'){
		switch (argv[*pos][1]){
			...
			case L'k': po->skipOutput = TRUE; break;
...
static void printArgs(ParserOpts *po){
	...
	wprintf(L"Skip if output exists:\t%s\n", po->skipOutput ? L"Yes" : L"No");
...
int wmain(int argc, wchar_t *argv[]){
	...
	wprintf(L"tSubGetC - Version: 0.9.7, Core version: %s (%s)\n\n",BUILD_VERSION, BUILD_DATE);
	if (argc < 2){
		printHelp(argv[0]);
		return 1;
	}
	...
	do{
		...
		if (ret != PARSER_OK){
			if (ret == PARSER_E_OUT_EXISTS){
				if (po.skipOutput) {
					wprintf(L"\nWarning: \"%s\" exists. Skipping.", po.fileOut);
					return 1;
				}
				wprintf(L"\nWarning: \"%s\" exists. Overwrite? Y/N [N]: ", po.fileOut);
				wchar_t res = towlower(getwchar());
				if (res == L'y')
